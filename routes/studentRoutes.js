...
router.post("/update_student/:email", ensureAuthenticated, async (req, res) => {
  ...
  try {
    ...
    const updateQuery = `
      ...
      SET first_name = $1, last_name = $2, mobile = $3, enrolment_no = $4, seat_type = $5, candidate_type = $6, college = $7, branch = $8, fee_status = $9, doa = $10
      WHERE email = $11
    `;
    const updateValues = [
      first_name,
    ...
    const htmlResponse = `
      <script>
        alert("An error occurred while updating student data: ${error.message}");
        setTimeout(function() {
          window.location.href = '/student_details';
        }, 0);
      </script>
